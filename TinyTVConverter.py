...
class TinyTVConverter(Frame):
    ...
    def initUI(self):
        ...
        self.selectTTVButton = Button(self.notebookFrame2, text="Connect to TinyTV", command=self.selectTTV, width=20)
        ...
        self.selectTTVButton.pack(side="top", expand=True)

        settingOptionsFrame.pack(side="top", expand=True)
        self.loopVideoSwitch.grid(row=1, column=3, sticky="we")
        self.liveVideoSwitch.grid(row=2, column=3, sticky="we")
        self.alphabetizeSwitch.grid(row=3, column=3, sticky="we")
        self.showStaticSwitch.grid(row=4, column=3, sticky="we")
        self.showChannelNumberSwitch.grid(row=5, column=3, sticky="we")
        self.showVolumeSwitch.grid(row=6, column=3, sticky="we")
        self.randomStartTimeSwitch.grid(row=7, column=3, sticky="we")
        self.randomStartChannelSwitch.grid(row=8, column=3, sticky="we")
        volumeLabelFrame.grid(row=9, column=3, sticky="we")
        self.volumeSlider.pack(expand=True, fill='both', padx=(5, 5), pady=(0,3))
        # self.selectTTVButton.grid(row=0, column=1, sticky="we")
        # importSettingsButton.grid(row=5, column=2, sticky="we")
        # exportSettingsButton.grid(row=6, column=2, sticky="we")
        ...
        loopVideoSwitchTip = CreateToolTip(self.loopVideoSwitch, 'Repeatedly loop current video')
        liveVideoSwitchTip = CreateToolTip(self.liveVideoSwitch, 'Imitate random tune-in time to video')
        alphabetizeSwitchTip = CreateToolTip(self.alphabetizeSwitch, 'Play videos in alphabetical order')
        staticEffectSwitchTip = CreateToolTip(self.showStaticSwitch, 'Imitate static effects on channel change')
        showChannelNumberTip = CreateToolTip(self.showChannelNumberSwitch, 'Show channel number briefly on channel change')
        showVolumeTip = CreateToolTip(self.showVolumeSwitch, 'Show volume slider on volume change')
        randomStartTimeTip = CreateToolTip(self.randomStartTimeSwitch, 'Randomize video playback time on startup')
        randomStartChannelTip = CreateToolTip(self.randomStartChannelSwitch, 'Randomize first video on startup')
        volumeSliderTip = CreateToolTip(self.volumeSlider, 'Set audio playback volume')
